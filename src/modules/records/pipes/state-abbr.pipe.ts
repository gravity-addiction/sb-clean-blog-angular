...
export class StateAbbrPipe implements PipeTransform {
    ...
    transform(value: string): string {
        const sLen = this.stateAbbrList.length;
        for (let i = 0; i < sLen; i++) {
            if (this.stateAbbrList[i].abbr.toLocaleLowerCase() === value.toLocaleLowerCase()) {
                return this.stateAbbrList[i].name + ' State';
            }
        }
        return 'National';
    }
